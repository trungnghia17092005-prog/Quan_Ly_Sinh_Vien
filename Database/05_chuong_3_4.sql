--xóa dữ liệu để cột để không bị trùng --
...
-- tạo tablespace để chứa bảng hoc phi --
...
-- tạo vpd chỉ mỗi sinh vien coi diem cua minh --
DBMS_OUTPUT.PUT_LINE('--- KIỂM THỬ VPD ---');
...
-- tạo vpd chỉ sinh vien coi lịch sử thanh toán --
SET SERVEROUTPUT ON;
DBMS_OUTPUT.PUT_LINE('chỉ xem được lịch sử đóng tiền');
EXEC Pkg_Security.Set_User('sv001');
SELECT
...
COMMIT;
-- giả lập xóa và khoi phục dữ liệu học phí --
...
INSERT INTO HocPhi (MaHP, MaSV, SoTien, NgayThanhToan, TrangThai, GhiChu, SoTKNH_Goc)
VALUES ('HP007', 'SV003', 7000000.00, SYSDATE, N'Da thanh toan', N'Học kì 4');
COMMIT;
